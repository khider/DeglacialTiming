...
% the probability of the cocos_uk record to lead the MD20_UP record across
% Termination III.
...
for i = 1 : size(age1,2)
    ...
    if nansum(loc1_target(:,i))==0
        loc1_target(:,i) = zeros(size(loc1_target,1),1);
    else
...
for i = 1 : size(age2,2)
    ind = find(age2(:,i) > startT & age2(:,i) < startT+d_min);
